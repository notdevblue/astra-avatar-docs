[v355 작업 폴더 정리](docs/maintenance/v355_WORKSPACE_CLEANUP_20260916.md) — 캐시·중복 제거와 과거 동작 무손실 압축, 파일 기준 순20.41GiB 축소. 현재 모델·수정 파일·Git 이력 보존.
...
사용자의 후속 실행 승인에 따라 전체 작업을 진행 중이다. 단추 공통 투영·카라 내부 선·바지 원화 배경 혼입·눈 깊이와 기존 중복 면·헤어 UV·Unity 재질/물리의 실제 시험과 독립 검토를 기록했다. 기존 아바타 텍스처를 새 채색 원본으로 사용하지 않으며 일본 애니메이션·게임·일러스트 NPR 스타일과 세부 선을 보존한다. 실패한 전개/물리 후보도 실제 사진과 미채택 이유를 함께 공개한다. 눈 바깥벽·머리 컬 방향·코트/넥타이/헤어 접촉·최종 통합 검수는 진행 중이며 최종 완료가 아니다. 모델/텍스처/실행 코드/원시 데이터는 이 문서 저장소에 포함하지 않는다.
...
아래 v352의 ‘목록 등록만’은 당시 결정 기록이며 현재 실행 상태를 뜻하지 않는다.
...
추가 대응: **P05-R 넥타이 중력·처짐**을 [사진 포함 목록](avatar_modeling/v352_eye_shadow_research/v352_REMAINING.md)에 등록했다. 몸 기울임 때 매듭 아래가 자연스럽게 처지는지 물리 활성·안정화 조건에서 별도 검수할 항목이며, 이번은 구현 전 목록 갱신이다.
...
이번 보고서의 T01–T14 번호는 v342 텍스처 이슈 번호이며 이 문서의 장기 추적 T번호와 별개입니다.
...
v344에서 최종 텍스처의 이동/바람 동작을 다시 실행하고 전후·확대 GIF를 추가했습니다. 리그는 변경하지 않았고 PhysBone 11개를 검사했습니다. [실제 검사와 한계](avatar_modeling/v344_texture_rig_retest/v344_WORK_REVIEW.md).
...
셰이더 v001 후속 정정: 사용자는 SiuSiu의 질감 표현을 선호한다고 설명했다. 테두리/띠 중심 해석과 E 자동후속기준을 보류한다. [사용자 피드백](avatar_shader/v001_siusiu_style/v001_FEEDBACK.md). 새 실험 없음.
