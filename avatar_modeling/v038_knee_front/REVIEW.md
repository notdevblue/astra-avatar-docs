...
`bianca_KNEE_FRONT_SMALL_REVIEW.blend`는 작은 보정 사본이다. 동작 확인용은 `bianca_KNEE_FRONT_MOTION_REVIEW.blend`다. 원본 v030을 덮어쓰지 않았으며 **사용자가 채택한 무릎 보정 기준 파일**이다.2026-09-09 사용자가 결과를 보고 “그래 좋다”라고 승인했다. 이후 관련 작업은 SMALL을 기준으로 이어간다. MEDIUM과 v037은 채택하지 않는다. 이번 결정으로 모델 파일이나 이미지를 변경하지 않았다.
