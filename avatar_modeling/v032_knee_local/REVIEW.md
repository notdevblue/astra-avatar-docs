...
`SUMMARY.json`, `preservation.json`, `verification_knee_*.json`, `verification_body_*.json`, `motion_reload.json`, 각 탐색 JSON에 원시 결과와 SHA가 있다. 스크립트는 상위 `knee_local_v032.py`, `knee_report_v032.py`. 실행 환경은 Blender5.2.1이며 그림은 기존 로컬 matplotlib 환경에서 생성했다. 새 외부 조사 대신 [v031 조사와 판단](../v031_topology/DIRECTION.md)을 실제 모델에 적용했다.
