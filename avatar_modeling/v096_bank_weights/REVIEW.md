...
**모두 미채택. 기준 v089 유지.** v094 조사 문서를 작성·커밋한 뒤 기존 모델 사본에서 수행했다. 높은 bank 3정점만 수정했다. BANK는 나머지 두 점의 4본 평균을 목표로 하고 ARM은 상완 영향 감소분을 spine에만 옮겼다.
...
기존 정점 좌표·모서리·면·UV·노멀·본·재질·이미지 등 fingerprint는 정확히 동일하다. 수정 정점 밖 웨이트는 변경하지 않았다. 중립의 평가 좌표 차이는 부동소수점 기준 최대 3.7252903e-09m, 웨이트 최대 변경량은 18.406484%p다. 새 메시·정점·면·본·텍스처0. 파일은 `bianca_<사본명>.blend`, 원시 검사 `CHECK_<사본명>.json`이다.
