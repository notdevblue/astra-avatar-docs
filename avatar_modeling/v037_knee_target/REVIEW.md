...
2026-09-09 사용자는 비교 결과를 보고 “아무리 봐도 v030이 더 좋다”고 판단했다. **v037은 미채택이며 다음 편집의 기본 파일로 사용하지 않는다. v030의 접힘 모양을 시각 기준으로 유지한다.** 아래의 검사 통과는 사용자 채택이나 자연스러운 형상 합격을 의미하지 않는다. 원본과 실험 파일은 기록으로 보존하며, 이번 피드백 뒤 모델을 추가 수정하지 않았다.

재평가하면 v037은 교차를 제거하는 과정에서 뒤쪽에 각진 턱이 생겼고, 정강이로 이어지는 윤곽과 접힌 끝의 형태가 달라졌다. v030은 앞쪽 두께 손실이 있어도 좁게 접히는 흐름이 더 자연스럽다. 뒤쪽까지 약11cm 높이 범위의 웨이트를 바꾼 것은 사용자가 원한 작은 무릎 보정의 시각적 범위를 벗어났다. 동일 정점 띠의 투영 폭 증가와 교차0을 형상 개선의 판정으로 확대했던 점을 바로잡는다.

차후 작업에서 v030의 뒤쪽 접힘을 고정 기준으로 삼아야 한다. 보이는 찢어짐·깜빡임 같은 렌더 문제와 내부 면 교차 수를 분리해 판단하고, 모든 접촉을 벌려 없애는 것을 목표로 삼지 않는다. 앞쪽 보정도 원본보다 나아진 동일 시점 비교가 먼저다. 이 문단은 방향 기록이며 새 수정본이나 사용자의 후속 모델 편집 승인을 뜻하지 않는다.

---

아래는 미채택 전의 실행·검증 기록이다.
